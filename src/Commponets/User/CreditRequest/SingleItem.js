...
const SingleItem = ({data, deleteHandle}) => {
    ...
    return (
        ...
        <td>{data?.toal_credit}</td> 
        
        {data?.paymentStatus === 0 ?<td className="badge badge-accent">Approved</td>  : <td className="badge badge-secondary"> Pendding </td> }
        ...
        {data?.status === 0 ?  <td>
         ...
         : 
         <td>     
         <Link className="btn btn-primary mr-1" to={`/user/credit/request/${data._id}`}>Credit History</Link>
     </td>  }
